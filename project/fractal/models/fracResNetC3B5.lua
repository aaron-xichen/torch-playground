...
local function createModel(opt)
    ...
    local function fracResC3(nInputPlane, nOutputPlane)
        assert(nInputPlaen == nOutputPlaen, 
            ('nInputPlane is not match nOutputPlane, %d -> %d'):format(nInputPlane, nOutputPlane))
        local right1 = nn.Sequential()
        :add(Convolution(nInputPlane, nOutputPlane, 3, 3, 1, 1, 1, 1))
        :add(SBatchNorm(nOutputPlane))
        :add(ReLU(true))
    ...
    if opt.dataset == 'cifar10' then
        ...
        for i=1,4 do
            ...
            for j=1,opt.nUnit-1 do
                model:add(fracResC3(nStages[i+1], nStages[i+1]))
            end
        ...
        for j=1,opt.nUnit-1 do
            model:add(fracResC3(nStages[6], nStages[6]))
        end
